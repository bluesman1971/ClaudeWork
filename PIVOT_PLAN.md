...
| 2026-03-06 | Hotfix: browser caching of stale JS after Railway redeploy. After Phase 4+5 was pushed, the new `index.html` deployed but old cached `main.js`/`generate.js`/`clients.js` ran from browser cache — causing `window.openGearProfileModal` to be undefined (gear profile modal didn't open) and "Please select at least one Dining preference" errors (old generate.js still had dining validation). Fixed by adding `NoCacheStaticFiles` subclass that sets `Cache-Control: no-cache, must-revalidate` on all `/src/*` static responses, and adding the same header to the root `FileResponse`. Also updated ARCHITECTURE.md, README.md, QUICKSTART.md to reflect current photography-only state. | — |
